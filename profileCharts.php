...
function profileCharts() {
	...
	$charts['majos']['query'] = "select (case when substr(report_value,1,6) = '10.11.' then 'El Capitan' when substr(report_value,1,6) = '10.10.' then 'Yosemite' when substr(report_value,1,5) = '10.9.' then 'Mavericks' when substr(report_value,1,5) = '10.8.' then 'Mountain Lion' when substr(report_value,1,5) = '10.7.' then 'Lion' when substr(report_value,1,5) = '10.6.' then 'Snow Leopard' when substr(report_value,1,5) = '10.5.' then 'Leopard' when substr(report_value,1,5) = '10.4.' then 'Tiger' when substr(report_value,1,5) = '10.3.' then 'Panther' else 'Other' end) p, count(*) c from reportRecord where report_key = 'osVersion' and report_id between $earliestReportID and $latestReportID and report_id in (select report_id from reportRecord where report_key = 'appName' and report_value=?) group by p order by c desc;";
